...
if [ "${NODE_NAME}" != "worker1" ] && [ "${NODE_NAME}" != "worker2" ]; then
    echo "[ERROR] This script should only run on worker1 or worker2."
    exit 1
fi
...
# --------------------------------------------------
# 7. Prepare Kubernetes Directory
# --------------------------------------------------
